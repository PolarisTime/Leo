...
- 系统：通用设置、jCasbin 纯 RBAC、附件与操作审计
...
- PostgreSQL 连通性和认证
- Redis 连通性和认证
- 可选启动密钥 `LEO_JWT_SECRET` / `LEO_DATA_ENCRYPTION_KEY`
...
MCP 不再接受 API Key。应创建独立机器账号，为其角色授予 `mcp_service:read` 及所需业务资源的 `read` 权限，再通过账号登录取得 JWT。每个 MCP 请求必须同时携带该 JWT 和与 JWT 用户 ID 一致的机器主体头：

```bash
Authorization: Bearer <machine-account-jwt>
X-Machine-Subject-Id: <machine-account-user-id>
```
...
- 不要把真实数据库密码、Redis 密码、JWT 密钥、数据加密密钥提交到仓库。
