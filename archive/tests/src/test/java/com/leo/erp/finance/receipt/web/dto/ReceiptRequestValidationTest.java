...
class ReceiptRequestValidationTest {
    ...
    @Test
    void shouldAllowSupplierReceiptWithoutCustomerName() {
        ReceiptRequest request = supplierReceiptRequest("", "项目名称");

        Set<String> violations = VALIDATOR.validate(request).stream()
                .map(violation -> violation.getPropertyPath().toString())
                .collect(Collectors.toSet());

        assertThat(violations).doesNotContain("customerName");
    }

    @Test
    void shouldAllowSupplierReceiptWithoutProjectName() {
        ReceiptRequest request = supplierReceiptRequest("客户名称", "");

        Set<String> violations = VALIDATOR.validate(request).stream()
                .map(violation -> violation.getPropertyPath().toString())
                .collect(Collectors.toSet());

        assertThat(violations).doesNotContain("projectName");
    }
    ...
    private static ReceiptRequest supplierReceiptRequest(String customerName, String projectName) {
        return new ReceiptRequest(
                ...
                "SUP-001",
                "供应商名称",
                ReceiptPurposes.SUPPLIER_OTHER_RECEIPT,
                null,
                null,
                customerName,
                null,
