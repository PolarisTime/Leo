...
class PurchaseInboundServiceTest {
    ...
    @Test
    void shouldReverseCompletedInboundToDraftWithoutChangingSourceOrder() {
        ...
        verify(completionSyncService, never()).synchronizeSourcePurchaseOrders(existing);
    }
    ...
    @Test
    void shouldLockExistingPurchaseOrderSourcesBeforeAuditingInbound() {
        PurchaseInboundRepository repository = mock(PurchaseInboundRepository.class);
        SourceAllocationLockService lockService = mock(SourceAllocationLockService.class);
        PurchaseInboundService service = lockAwareService(
                repository,
    ...
    @Test
    void shouldRejectAuditingInboundThroughGenericStatusEndpoint() {
        PurchaseInboundRepository repository = mock(PurchaseInboundRepository.class);
        PurchaseInboundService service = lockAwareService(
                repository,
                mock(SnowflakeIdGenerator.class),
        ...
        assertThatThrownBy(() -> service.updateStatus(1L, StatusConstants.AUDITED))
                ...
                .hasMessageContaining("必须使用专用审核接口");

        verify(repository, never()).save(any());
    }

    @Test
    void shouldRejectAuditingDraftInboundWhenAuditedRefundHasConsumedItsQuantity() {
        PurchaseInboundRepository repository = mock(PurchaseInboundRepository.class);
        ...
        when(refundSummary.getTotalQuantity()).thenReturn(8L);
        when(refundRepository.summarizeAuditedQuantityBySourcePurchaseOrderItemIds(List.of(201L)))
                .thenReturn(List.of(refundSummary));

        assertThatThrownBy(() -> service.audit(1L, List.of()))
                .isInstanceOf(BusinessException.class)
                .hasMessageContaining("采购退款单")
                .hasMessageContaining("不能审核");

        assertThat(existing.getStatus()).isEqualTo("草稿");
        verify(repository, org.mockito.Mockito.never()).save(any());
    }

    @Test
    void shouldRejectUnauditingInboundWhileAuditedRefundDependsOnIt() {
        PurchaseInboundRepository repository = mock(PurchaseInboundRepository.class);
        PurchaseRefundRepository refundRepository = mock(PurchaseRefundRepository.class);
        SourceAllocationLockService lockService = mock(SourceAllocationLockService.class);
        PurchaseInboundService service = lockAwareService(
        ...
        when(repository.findByIdAndDeletedFlagFalse(1L)).thenReturn(Optional.of(existing));
        when(refundSummary.getSourcePurchaseOrderItemId()).thenReturn(201L);
        when(refundSummary.getTotalQuantity()).thenReturn(6L);
        when(refundRepository.summarizeAuditedQuantityBySourcePurchaseOrderItemIds(List.of(201L)))
                .thenReturn(List.of(refundSummary));
    ...
    @Test
    void shouldCompleteInboundWithoutCompletingSourcePurchaseOrderWhenAuditing() {
        PurchaseInboundRepository repository = mock(PurchaseInboundRepository.class);
        PurchaseInboundCompletionSyncService completionSyncService =
                mock(PurchaseInboundCompletionSyncService.class);
        PurchaseInboundService service = lockAwareService(
    ...
    @Test
    void shouldRejectCreatingPurchaseInboundAsAudited() {
        PurchaseInboundRepository repository = mock(PurchaseInboundRepository.class);
        SnowflakeIdGenerator idGenerator = mock(SnowflakeIdGenerator.class);
        PurchaseInboundCompletionSyncService completionSyncService =
                mock(PurchaseInboundCompletionSyncService.class);
        ...
        PurchaseInboundRequest draft = request();
        PurchaseInboundRequest audited = new PurchaseInboundRequest(
                draft.inboundNo(),
                draft.purchaseOrderNo(),
                draft.supplierName(),
                draft.warehouseName(),
                draft.inboundDate(),
        ...
        when(idGenerator.nextId()).thenReturn(1L);
        assertThatThrownBy(() -> service.create(audited))
                .isInstanceOf(BusinessException.class)
    ...
    @Test
    void shouldSaveOverToleranceDraftButRejectAuditWithoutConfirmation() {
        PurchaseInboundRepository repository = mock(PurchaseInboundRepository.class);
        PurchaseInboundMapper mapper = mock(PurchaseInboundMapper.class);
        TradeItemMaterialSupport materialSupport = mock(TradeItemMaterialSupport.class);
        WarehouseSelectionSupport warehouseSelectionSupport = mock(WarehouseSelectionSupport.class);
        ...
        var inboundCaptor = forClass(PurchaseInbound.class);
        verify(repository).save(inboundCaptor.capture());
        PurchaseInbound saved = inboundCaptor.getValue();
        when(repository.findByIdAndDeletedFlagFalse(saved.getId())).thenReturn(Optional.of(saved));

        assertThatThrownBy(() -> service.audit(saved.getId(), List.of()))
                .isInstanceOf(BusinessException.class)
                .hasMessageContaining("过磅重量上差超出允许范围")
                .hasMessageContaining("必须选择超差原因");

        PurchaseInboundItem savedItem = saved.getItems().getFirst();
        service.audit(saved.getId(), List.of(
                new com.leo.erp.purchase.inbound.web.dto.PurchaseInboundAuditRequest.OverToleranceConfirmation(
                        savedItem.getId(),
                        "SUPPLIER_CONFIRMED",
                        "供应商确认实磅"
                )
        ...
        assertThat(savedItem.getToleranceDirection()).isEqualTo("上差");
        assertThat(savedItem.getToleranceLimitPercent()).isEqualByComparingTo("5.0000");
        assertThat(savedItem.getToleranceActualPercent()).isEqualByComparingTo("5.7143");
        assertThat(savedItem.getToleranceReasonCode()).isEqualTo("SUPPLIER_CONFIRMED");
        assertThat(savedItem.getToleranceConfirmedAt()).isNotNull();
    }
    ...
    void shouldWriteBackActualWeighTotalWhenPurchaseOrderItemFullyInbound() {
        ...
        assertThat(loadedSourceItem.getWeightTon()).isEqualByComparingTo("14.700");
        assertThat(loadedSourceItem.getActualWeightTon()).isEqualByComparingTo("14.258");
        assertThat(loadedSourceItem.getActualPieceWeightTon()).isNull();
        assertThat(loadedSourceItem.getAmount()).isEqualByComparingTo("44100.00");
        assertThat(loadedPurchaseOrder.getTotalWeight()).isEqualByComparingTo("14.700");
        assertThat(loadedPurchaseOrder.getTotalAmount()).isEqualByComparingTo("44100.00");
    }

    @Test
    void shouldAllowDraftPurchaseInboundWhenWeighRequiredCategoryMissingWeighWeight() {
        PurchaseInboundRepository repository = mock(PurchaseInboundRepository.class);
        PurchaseInboundMapper mapper = mock(PurchaseInboundMapper.class);
        TradeItemMaterialSupport materialSupport = mock(TradeItemMaterialSupport.class);
        WarehouseSelectionSupport warehouseSelectionSupport = mock(WarehouseSelectionSupport.class);
        ...
        assertThat(request.items().getFirst().weighWeightTon()).isNull();
    }

    @Test
    void shouldRejectAuditedPurchaseInboundWhenWeighRequiredCategoryMissingWeighWeight() {
        PurchaseInboundRepository repository = mock(PurchaseInboundRepository.class);
        PurchaseInboundService service = lockAwareService(
                repository,
                mock(SnowflakeIdGenerator.class),
        ...
        PurchaseInbound existing = inbound();
        PurchaseInboundItem item = inboundItem(101L, existing, 4);
        item.setSettlementMode("过磅");
        ...
        assertThatThrownBy(() -> service.audit(1L, List.of()))
                .isInstanceOf(BusinessException.class)
                .hasMessageContaining("第1行需填写大于0的过磅重量后才能审核");
    ...
    @Test
    void shouldKeepPurchaseOrderAuditedUntilExplicitCompletionCommand() {
        PurchaseInboundRepository repository = mock(PurchaseInboundRepository.class);
        PurchaseInboundMapper mapper = mock(PurchaseInboundMapper.class);
        TradeItemMaterialSupport materialSupport = mock(TradeItemMaterialSupport.class);
        WarehouseSelectionSupport warehouseSelectionSupport = mock(WarehouseSelectionSupport.class);
        ...
        assertThat(completedInbound.getStatus()).isEqualTo("草稿");
