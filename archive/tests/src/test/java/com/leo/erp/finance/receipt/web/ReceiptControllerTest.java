...
class ReceiptControllerTest {
    ...
    void pageReturnsPaginatedReceipts() {
        ...
        ApiResponse<PageResponse<ReceiptResponse>> response = controller.page(
                query, "test", "customer", "供应商", null, "active", null, null
        );

        assertThat(response.code()).isEqualTo(0);
        assertThat(response.data().content()).hasSize(1);
        ArgumentCaptor<PageFilter> filterCaptor = ArgumentCaptor.forClass(PageFilter.class);
        verify(receiptService).page(eq(query), filterCaptor.capture());
        assertThat(filterCaptor.getValue().businessType()).isEqualTo("供应商");
    }
