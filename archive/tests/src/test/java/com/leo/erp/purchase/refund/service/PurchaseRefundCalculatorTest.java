...
class PurchaseRefundCalculatorTest {
    ...
    @Test
    void shouldRefundOnlyUnreceivedQuantityAtTheoreticalBasisDespiteMeasuredWeightDifference() {
        ...
        PurchaseRefundCalculator.Calculation result = calculator.calculate(order, List.of(inboundItem));

        assertThat(result.lines()).hasSize(1);
        assertThat(result.lines().getFirst().quantity()).isEqualTo(2);
        assertThat(result.lines().getFirst().weightTon()).isEqualByComparingTo("4.00000000");
        assertThat(result.lines().getFirst().amount()).isEqualByComparingTo("12000.00");
    }

    @Test
    void shouldNotRefundMeasuredWeightDifferenceWhenOrderedQuantityWasFullyReceived() {
    ...
    @Test
    void shouldIgnoreWrittenBackOrderWeightWhenQuantityWasFullyReceived() {
    ...
    @Test
    void shouldNotCreateRefundForFullyReceivedRoundedWeightDifference() {
    ...
    @Test
    void shouldIgnoreCentDifferenceWhenQuantityWasFullyReceived() {
        PurchaseOrderItem sourceItem = sourceItem(101L, 1, 1, "0.00500000", "1.00");
        sourceItem.setPieceWeightTon(new BigDecimal("0.00500000"));
        PurchaseInboundItem inboundItem = inboundItem(
                sourceItem,
