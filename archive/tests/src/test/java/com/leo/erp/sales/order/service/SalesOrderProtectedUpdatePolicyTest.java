...
class SalesOrderProtectedUpdatePolicyTest {
    ...
    @Test
    void shouldRejectAuditedOrderReturningDraftThroughOrdinaryUpdate() {
    ...
    @Test
    void shouldRejectCompletingDeliveryVerificationThroughOrdinaryUpdate() {
        SalesOrder order = auditedSalesOrder();
        order.setStatus(StatusConstants.DELIVERY_VERIFICATION);
    ...
    @Test
    void shouldRejectAuditedUpdateWhenAnyProtectedHeaderFieldChanges() {
        SalesOrder order = auditedSalesOrder();

        assertRejected(order, "orderNo", requestWith(order, StatusConstants.AUDITED, builder -> builder.orderNo = "SO-CHANGED"));
        assertRejected(order, "purchaseInboundNo", requestWith(order, StatusConstants.AUDITED, builder -> builder.purchaseInboundNo = "PI-CHANGED"));
        assertRejected(order, "purchaseOrderNo", requestWith(order, StatusConstants.AUDITED, builder -> builder.purchaseOrderNo = "PO-CHANGED"));
        assertRejected(order, "customerCode", requestWith(order, StatusConstants.AUDITED, builder -> builder.customerCode = "C999"));
        assertRejected(order, "customerId", requestWith(order, StatusConstants.AUDITED, builder -> builder.customerId = 1002L));
        assertRejected(order, "customerName", requestWith(order, StatusConstants.AUDITED, builder -> builder.customerName = "客户B"));
        assertRejected(order, "projectId", requestWith(order, StatusConstants.AUDITED, builder -> builder.projectId = 202L));
        assertRejected(order, "projectName", requestWith(order, StatusConstants.AUDITED, builder -> builder.projectName = "项目B"));
        assertRejected(order, "salesName", requestWith(order, StatusConstants.AUDITED, builder -> builder.salesName = "李四"));

        assertThat(policy.allowsProtectedUpdate(
                order,
                requestWith(order, StatusConstants.AUDITED, builder -> {
                    builder.deliveryDate = order.getDeliveryDate().plusDays(1);
                    builder.remark = "新备注";
                })
        )).isTrue();
    ...
    @Test
    void shouldRejectAuditedUpdateWhenRequestItemsAreNull() {
        SalesOrder order = auditedSalesOrder();
        SalesOrderRequest request = requestWith(order, StatusConstants.AUDITED, builder -> builder.items = null);

        assertThat(policy.allowsProtectedUpdate(order, request)).isFalse();
    }

    @Test
    void shouldRejectAuditedUpdateWhenAnyProtectedItemFieldChanges() {
        SalesOrder order = auditedSalesOrder();
        ...
        assertThat(policy.allowsProtectedUpdate(
                order,
                requestWithItem(order, builder -> {
                    builder.unitPrice = new BigDecimal("3001.00");
    ...
    @Test
    void shouldAllowAuditedUpdateWhenNumericScaleAndBlankQuantityUnitNormalizeToSameValues() {
        SalesOrder order = auditedSalesOrder();
    ...
    private SalesOrderRequest requestWithItem(SalesOrder order, Consumer<SalesOrderItemRequestBuilder> customizer) {
        SalesOrderItem item = order.getItems().get(0);
        return requestWith(order, StatusConstants.AUDITED, builder ->
