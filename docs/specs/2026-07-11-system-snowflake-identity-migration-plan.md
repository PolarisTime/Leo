...
| 项目 | 内容 |
| --- | --- |
| 状态 | In Progress；生产切换已完成：当前应用指向由实时 V11 备份恢复并按 `V12–V29 → D3 → V30–V47 → D4 → V48–V56` 迁移校验的副本，旧 `Master_Prod` 保留为 V11 回滚库；最新可复核的完整 Playwright E2E 仍为 25 passed、57 failed、2 skipped，认证已通过但全量门禁未通过；后续修复后的运行时观察未形成独立完整归档，不计入发布门禁 |
| 计划版本 | 3.1（V56 隔离验证与 E2E 证据校正版） |
| 日期 | 2026-07-13 |
...
历史实现差异已提交到本地 Git但尚未推送；本次生产数据修复线和测试隔离修复仍保留在工作树。下表中的“完成”只代表冻结工作树上的本地实现和本地自动化证据，
不代表已合并、已部署或已通过生产发布门禁。

| 工作包 | 当前状态 | 已有证据 | 进入下一阶段前仍需完成 |
| --- | --- | --- | --- |
| G0 雪花根基 | 本地实现完成 | 预分配失败关闭、本机生成、Long 字符串 JSON 与更新 ID 不变契约纳入后端全量 6184 项 | 生产机器号清单、时钟回拨告警与部署配置核验 |
| G1 客户/项目错写止血 | 本地实现完成 | 客户/项目分别解析，ID 碰撞、跨客户、同名不同 ID 与快照冲突回归通过 | 历史错绑只读扫描、业务清洗确认、共享环境验证 |
| E1/B1/C1 数据迁移 | V20–V56 本地脚本与契约完成；两份生产快照隔离副本按阶段迁移至 V55，V56 仅在开发测试副本验证 | Final Contract 5/5、Forward Contract 9/9、Constraint Contract 8/8、checksum 2/2；最新候选库主线 55/55、release repair baseline + D3 + D4 共 3/3，开发测试库主线 V56/56；失效索引和未解析身份均为 0；空库 V1→V55 migrate/validate 通过 | 正式生产批准 D3/D4 manifest 与身份映射、V56 代码评审、锁/WAL/空间评估、共享环境分阶段迁移和数据守恒报告 |
| A1 后端双写/读模型 | 本地实现完成 | 物流稳定来源 91/91；供应商候选 126/126；客户/项目/物流商候选 128/128；空 V55 库后端全量 6184/6184 | 历史数据、权限和真实状态流的共享环境验证；生产 fallback 观察 |
| A2 前端 API/交互链 | 本地实现完成 | 三类 Statement 6 文件 109/109；Vitest 6181 passed、4 skipped；typecheck、Biome/ESLint、生产构建通过；最新可复核完整 E2E 为 25 passed、57 failed、2 skipped | 归档并重跑修复后的完整 E2E；共享环境完成保存/回读、真实权限与软删除行为验证 |
| QA 本地门禁 | 完成 | 当前 703 个测试源编译；D3/D4 release 契约 8/8；18 类联合 62/62；空 V55 库 `mvn test` 6184/6184，`verify`、Checkstyle、JaCoCo 均通过；两仓 diff check 通过 | 现有 SpotBugs 51 个高等级告警另行治理，不冒充已通过；修复 E2E 契约并补齐发布级门禁 |
| 发布门禁 | 未完成 | 生产快照黄金副本迁移、应用冒烟和未认证 403 门禁已通过；开发测试副本 V56 启动验证通过，最新可复核完整 E2E 仍为 25 passed、57 failed、2 skipped | 正式生产 repair 映射、机器号/时钟、历史清洗、真实 `pg_index`、共享环境、fallback 完整周期、Dashboard/告警/回滚、完整 E2E 归档重跑、正式发布与部署 |
...
当前剩余项全部属于发布级或外部环境门禁：生产雪花机器号与时钟配置；历史数据只读扫描、人工清洗和守恒归档；真实
`pg_index` 的列序、谓词、重复索引与锁影响复核；受限或匿名化生产副本的迁移耗时、锁等待、WAL 与索引空间演练；共享环境
`migrate/validate`、应用冒烟与真实状态流；生产 fallback 连续完整业务周期为 0；Dashboard、告警、备份恢复和回滚演练；修复现有 E2E selector/fixture 契约并全量重跑；
正式提交、合并、发布和部署。现有 `test:e2e:mock` 只启动 Vite并跳过后端健康检查，仍把 `/api` 代理到
`127.0.0.1:11211`，没有覆盖稳定身份的共享 API mock，因此后端未启动时系统性 `ECONNREFUSED`；该项不得标记为通过。

因此当前可认定“稳定身份本地代码与本地迁移/质量门禁完成”，但不能认定全系统或生产已经固定。

### 1.2 本次续作收口点（截至 2026-07-13）

| 项目 | 当前状态 | 下一动作 |
| --- | --- | --- |
| 采购/销售完成状态同步 | 本地实现与全量回归完成；采购 91 项、销售订单/出库 213 项定向通过 | 在共享环境验证真实业务状态流转 |
| 物流稳定来源与下游保护 | 本地实现与全量回归完成；来源明细 ID、按 ID锁源、占用和快照继承 91/91；下游保护 6/6 | 在共享环境完成物流/对账/财务跨域冒烟 |
| 候选稳定 ID | 本地实现与全量回归完成；供应商 126/126、客户/项目/物流商 128/128、销售 117/117 | 在共享环境验证分页 total、权限和真实占用 |
| 前端稳定身份链 | 本地实现与单元/构建门禁完成；Statement 109/109，Vitest 6181 passed + 4 skipped，typecheck/lint/build/audit 通过；最新可复核完整 E2E 为 25 passed、57 failed、2 skipped | 归档并重跑修复后的完整 E2E，确认采购订单导入、附件 smoke、API Key 详情候选数据，再完成共享环境保存/回读 |
| V20–V55 冻结证据 | 两份独立 SHA-256 manifest、迁移契约、空 V55 库、历史 D1/D2 黄金副本和最新 D3/D4 候选库均通过；release repair SQL/契约仍未提交 | 业务批准正式 D3/D4 manifest 与身份映射，完成代码评审与远端合并；在共享环境复核 |
| 本地全量门禁 | 后端空 V55 库 6184/6184 + verify、前端 6181 passed + 4 skipped、本地构建/审计、两仓 diff check 均通过；开发测试副本 V56 与认证链通过，但完整 E2E 仍为 25/57/2 | 归档并重跑修复后的完整 E2E，核对采购订单导入、附件 smoke、live API Key 候选后再完成生产/共享环境发布门禁；不复用未归档运行时观察 |
...
2. **L2 关系固定**：所有跨实体和跨单据关系都保存目标雪花 ID，并由类型校验/FK保护。本地需求范围已大范围收口，V38–V55 已在本地验证；生产历史数据和目标环境约束尚未验证。
3. **L3 读模型固定**：库存、应收应付、候选、容量和导出都按 ID计算，不再按名称、编码或单号猜测。本地核心读模型、候选和容量链已收口并通过全量测试；共享环境真实数据验证尚未完成。
4. **L4 契约固定**：前端、API、迁移、观测和发布门禁全部保持字符串 ID，兼容回退清零并移除。本地前后端与迁移门禁已通过，但生产 fallback 周期、E2E、观测和发布门禁尚未完成。
...
| 领域 | 当前内部身份 | 本地关联方式 | 本地状态 | 剩余发布风险 |
| --- | --- | --- | --- | --- |
| 业务实体主键 | 雪花 `id` | 主表主键 | 已固定 | 无统一问题 |
| 采购/销售来源明细 | 来源头/明细雪花 ID + 单号快照 | 来源、容量、占用和完成状态按 ID；单号只展示/检索 | 本地固定 | 生产孤儿/空来源预检与真实状态流尚未完成 |
| 结算主体 | 主数据雪花 ID + 名称快照 | 业务、财务和报表按 `settlement_company_id` 隔离 | 本地固定 | 目标环境历史空值、删除保护和约束需复核 |
| 供应商 | `supplier_id` + 编码/名称快照 | 采购、收票、对账、付款、退款和候选按 ID | 本地固定 | 生产历史回填、fallback 清零和权限验证 |
| 物流商 | `carrier_id` + 编码/名称快照 | 物流、车辆、对账、付款和候选按 ID | 本地固定 | 生产车辆归属数据与 V48/V49 约束验证 |
| 客户 | `customer_id` + 编码/名称快照 | 销售、出库、物流明细、开票、对账、收款和候选按 ID | 本地固定 | 历史错绑清洗与生产 fallback 清零 |
| 项目 | `project_id` + 名称快照 | 按真实项目 ID并校验客户归属；候选按 `customerId/projectId` | 本地固定 | 历史项目客户归属和生产数据验证 |
| 商品/SKU | `material_id` + 商品快照 | 单据明细、来源链、库存和报表按 ID | 本地固定 | 商品身份属性业务决策与生产守恒报告 |
| 仓库 | `warehouse_id` + 名称快照 | 入出库明细、库存和报表按 ID | 本地固定 | 仓库合并策略与生产守恒报告 |
| 批次 | 暂无独立实体，批次号是业务维度 | 批次号字符串 + 来源明细 | 待收紧 | 需先规范化并禁止生效后改号；仅在确有独立生命周期时新增库存层实体 |
| 发票/对账/收付款 | 单据与 typed party/source 雪花 ID | 直接来源、分配、占用和候选按 ID；快照仅展示 | 本地固定 | 生产历史兼容命中和 fallback 周期未验证 |
| 报表 | 聚合行使用稳定 ID维度键 | 库存、出入库、应收应付按实体/结算主体 ID聚合 | 本地固定 | 生产数据守恒与 SQL 执行计划未归档 |
| 系统/认证/附件 | 实体 BIGINT ID，普通关系使用 FK | V50–V52 本地补索引并验证 8 条普通 FK；协议键和多态 `record_id` 明确排除 | 本地固定 | 共享/生产孤儿扫描、删除语义和锁影响未验证 |

### 4.1 已发现并在当前工作区止血的 P0 身份错写
...
则会静默绑定错误项目。当前工作区已经改为分别写入 `customerId` 与 `projectId`，并增加 ID碰撞、跨客户、同名项目和快照冲突回归测试；
在代码完成受控提交、部署并完成历史异常清洗前，仍不得把该风险标记为生产已关闭。

审计基线中的 `md_project` 主要保存 `customer_code`，没有完整的 `customer_id` 外键关系；当前工作树已通过 V20增加
`customer_id`，由后续回填/约束版本和服务校验固定客户与项目归属，编码和名称仅保留为快照或检索字段。
...
| `st_freight_statement_item` | 新增 `source_freight_bill_id`、`source_freight_bill_item_id` 及客户/项目/商品/仓库 ID | 直接物流单明细；销售出库来源 ID仅作更远端追踪 | 以 `(source_freight_bill_item_id, source_freight_bill_id)` 组合 FK保证同一父项，删除文本猜测；V35–V37 已在 `leo_test` 和临时空库执行，尚未在共享/生产环境执行 |
...
当前本地实现已由 V50–V52 为已审计的系统普通关系补齐支撑索引、增加并验证恰好 8 条 FK；
`module_key + record_id` 等多态关系不建立错误的普通 FK。车辆/物流商组合归属由 V48–V49 增加并验证，
采购入库直接来源明细由 V53–V55 分阶段验证后收紧为 `NOT NULL`。这些结论已在 `leo_test` V55和临时空库
V1→V55 验证，但不替代共享/生产环境的孤儿扫描、删除语义评审和锁影响复核。
...
- 当前工作区 V20–V56 已完成跨领域契约串联，并按冻结迁移文件管理；V20–V46 与 V47–V55 的独立 SHA-256 快照已建立，V56 为本地未提交的兼容前滚，后续修正只能从 V57 前滚。
...
V20–V55 文件当前已形成完整本地版本线并纳入本地 Git提交，但尚未推送或部署；为避免后续环境产生不同 checksum，
...
清单不能证明文件在快照建立前从未改写；当前本地提交仍需经过代码评审和远端合并。以下内容记录实际文件与后续 V57+ 前滚边界。

| 范围 | 当前状态 | 执行许可 |
| --- | --- | --- |
| V20 | 文件存在，且已有测试执行证据 | 不得改写；问题用 V57+ 前滚 |
| V21–V37 | Expand、Backfill、索引和物流对账补缺文件存在 | 文件冻结；共享环境执行仍受发布 A/B 数据门禁约束 |
| V38–V46 | FK、CHECK、Validate、NOT NULL 和前滚补缺文件存在 | 文件冻结；生产数据预检、兼容发布和副本演练前禁止执行 |
| V47 | 3 个既有 FK 的全量左前缀索引文件存在 | 文件冻结；真实 schema 与锁影响复核前不得在共享环境执行 |
| V48–V55 | 车辆归属、8 条系统普通关系 FK和采购入库直接来源收紧文件存在；本地契约、`leo_test` 与空库验证通过 | 文件冻结；生产数据预检、真实 `pg_index`、副本演练和兼容发布前不得在共享环境执行 |
| V56 | `allow_delivery_verification_sales_order_status` 已创建，且仅在隔离开发测试库验证通过；未提交、未在候选库或生产执行 | 保持文件冻结；正式共享/生产执行前需完成代码评审、空值与状态流验证及发布审批 |
...
**已形成的补缺、Constraint、Validate 与 Contract**

| 实际版本/发布 | 单一职责 | 进入条件/当前状态 |
| --- | --- | --- |
| V35 | `expand_freight_statement_item_party_identity`：前滚增加物流对账明细 `customer_id/project_id` 等已确认遗漏列 | 本地文件、`leo_test` 和空库执行完成；不修改 V20–V34 |
| V36 | `add_constraint_supporting_indexes`：补 V26 覆盖矩阵遗漏的全量 FK索引和组合索引 | 本地文件、`leo_test` 和空库执行完成；仍需在真实 schema 检查冗余和锁影响 |
| V37 | `backfill_freight_statement_item_party_identity`：从直接物流单明细继承客户/项目 ID | 本地执行完成；共享/生产零候选、多候选、跨客户项目仍必须为 0 |
| 发布 B | 库存、应收应付、候选、容量和占用校验全部切换为 ID读取 | 生产双写与 V27–V37 回填逐域核对通过 |
| V38 | `add_stable_identity_foreign_keys`：按领域增加主数据、来源、结算主体 FK | 本地文件、契约和迁移验证完成；共享/生产 orphan anti-join 和索引覆盖必须为 0缺口 |
| V39 | `add_stable_identity_checks`：增加 typed party、来源互斥和必需身份辅助 CHECK | 本地文件、契约和迁移验证完成；共享/生产历史行待验证 |
| V40 | `validate_stable_identity_foreign_keys`：验证 V38 FK | 本地验证完成；共享/生产任一失败停止发布 |
| V41 | `validate_stable_identity_checks`：验证 V39 CHECK | 本地验证完成；共享/生产任一失败停止发布 |
| V42 | `enforce_stable_identity_not_null`：收紧已验证的 party、商品、仓库、来源和结算字段 | 本地执行完成；共享/生产只能在发布 B稳定且旧写入停止后执行 |
| V43 | `add_missing_stable_identity_fk_indexes`：补充组合和直接来源 FK索引 | 本地执行完成；3 个剩余全量左前缀索引缺口由 V47补齐，`fm_payment(source_purchase_order_id)` 已由 V36覆盖 |
| V44 | `add_missing_stable_identity_checks`：补直接来源及台账 typed party CHECK | 本地执行完成；共享/生产历史行待验证 |
| V45 | `validate_missing_stable_identity_checks`：验证 V44 CHECK | 本地验证完成；共享/生产任一失败停止发布 |
| V46 | `enforce_required_direct_source_not_null`：收紧客户/供应商对账及收/开票直接来源 | 本地执行完成；采购入库直接来源后续已由 V53–V55覆盖 |
| 发布 C | 请求 ID 必填，停止旧字段身份回退 | fallback 连续一个完整业务周期为 0 |
| 后续清理（最早 V57） | 只移除已确认无命中的名称/编码/单号身份回退和无用索引；永久保留快照 | V47–V56 补缺完成、发布 C 完成且具备回滚证据；版本号以届时最大版本 + 1 为准 |

**V47–V55 本地 Contract 补缺完成**

| 建议版本 | 单一职责 | 前置检查与验收 |
| --- | --- | --- |
| V47 | 补 `po_purchase_refund(source_purchase_order_id)`、`fm_supplier_refund_receipt(purchase_refund_id)`、`sys_role_conflict(conflict_role_id)` 三个全量左前缀索引；不得重复创建 V36 已有的 `fm_payment(source_purchase_order_id)` 索引 | 本地文件与严格契约完成；恰好 3 条普通 `CREATE INDEX`。共享环境执行前仍需在真实 `pg_index` 复核列序、谓词、重复索引与锁影响 |
| V48 | 建立 `md_vehicle(id, carrier_id)` 唯一索引，并为 `lg_freight_bill(vehicle_id, carrier_id)` 建组合索引、`NOT VALID` 组合 FK及车辆 ID/车牌同空同非空 CHECK | 本地文件与契约完成；`leo_test`/空库预检和迁移通过。共享/生产前仍需确认跨物流商车辆、重复车辆和快照空值为 0 |
| V49 | 只验证 V48 车辆归属 FK/CHECK | 本地验证完成；共享/生产任一跨物流商引用仍必须停止发布 |
| V50 | 为认证、用户角色、角色权限、附件绑定、部门/角色树等普通关系补全量支撑索引 | 本地文件与契约完成；`module_key + record_id` 多态关系和审计 actor 不进入普通实体 FK |
| V51 | 为 V50 对应普通关系增加恰好 8 条 `NOT VALID` FK | 本地文件与契约完成；共享/生产前 orphan anti-join 必须为 0，删除动作逐表评审，禁止统一 CASCADE |
| V52 | 只验证 V51 系统关系 FK | 本地验证完成；共享/生产验证失败仍须停止发布并输出孤儿清单 |
| V53 | 为 `po_purchase_inbound_item.source_purchase_order_item_id` 增加 `NOT VALID` 非空辅助 CHECK | 本地文件与契约完成；共享/生产前历史空来源、孤儿和来源头冲突必须为 0 |
| V54 | 只验证 V53 采购入库直接来源 CHECK | 本地验证完成；PostgreSQL 集成夹具均建立真实采购订单明细来源 |
| V55 | 将采购入库直接来源列设为 `NOT NULL` 并移除辅助 CHECK | 本地 `leo_test`/空库执行完成；共享/生产仍要求发布 B稳定、旧写入停止和生产副本验证 |

本地数据库证据：`leo_test` 从 V47 顺序执行至 V55并成功 validate 55 项；临时 PostgreSQL 16.14 空库从 V1顺序执行至 V55，
`migrate` 与 `validate` 均成功且临时容器已销毁。该证据只证明当前文件链在本地数据和空库可执行，不能替代匿名化生产副本、
共享环境、真实锁等待/WAL/索引空间或业务守恒验证。
...
- 数据库：本地 `leo_test` 当前为 V55并成功 validate 55 项；临时 PostgreSQL 16.14 空库 V1→V55 migrate/validate 55 项成功；PostgreSQL 18.4 空 V55 库和历史黄金副本证据见第 1.3 节；最新 D3/D4 候选库保持 V55，`master_prod_devtest_20260713` 另已校验并运行 V56，三条 Flyway 线证据见第 1.6 节。
...
- 静态检查：`leo`、`aries` 的 `git diff --check` 和两个部署触发脚本 `bash -n` 通过；最新 E2E canonical locator 与认证会话 helper 的目标 Biome、ESLint、TypeScript 检查通过，但修复后的完整 Playwright 结果尚无独立归档。

未通过或未执行的发布门禁：受限生产快照黄金副本和最新 D3/D4 候选库的数据库迁移与完整性扫描已通过；历史真实 Playwright E2E 依次为 8 passed、76 failed、0 skipped 和 25 passed、57 failed、2 skipped，后续修复后的运行时观察未形成独立完整归档，不能替代全量门禁。生产副本的锁/WAL/空间与耗时评估、业务余额/库存守恒、共享环境、生产 fallback 周期、
Dashboard/告警、备份恢复、回滚演练和正式发布/部署仍未执行。SpotBugs 仍有 51 个既有高等级告警，不能写成已通过。

## 12. 验收标准

全部满足后，才能对外宣称“全系统稳定身份固定完成”：

当前第 9 项本地单元、静态与构建质量门禁已满足，第 10 项已完成受限生产快照副本的迁移、完整性扫描和应用冒烟但尚缺业务余额/库存守恒；最新可复核完整 E2E 仍为 25 passed、57 failed、2 skipped，修复后的运行时观察未形成独立归档，仍需重新执行并完成共享环境与正式发布级门禁；
第 1–8、10–12 项的其余条件仍需业务批准、共享环境、fallback 或正式发布证据，
...
2026-07-13 D3/D4 也只在候选库执行。当前未授权生产 target 或部署，因此保持失败关闭，禁止触发自动发布。
...
当前工作区已经完成“本地代码、迁移草案与本地质量门禁闭环”并创建本地提交，未推送且未执行共享环境迁移。后续状态按本地完成与发布待验证分离：

| 任务 ID | 范围 | 当前状态 | 具体产物 | 完成门禁 |
| --- | --- | --- | --- | --- |
| DB-ID-01 | V20–V56 冻结迁移审查 | V20–V55 已冻结，V56 未提交且仅隔离验证 | 两份独立 checksum manifest；Final 5/5、Forward 9/9、Constraint 8/8；`leo_test` V55、开发测试库 V56和空库 V1→V55 | 代码评审与远端合并；真实 `pg_index`、匿名化生产副本与共享环境验证 |
| DB-ID-02 | V35–V37 物流对账补缺 | 本地完成 | 物流对账明细客户/项目前滚补列、索引、回填，并已进入 V1→V55 空库链 | 共享/生产来源继承、跨客户项目和空值预检为 0 |
| DB-ID-03 | 生产快照 repair 与候选库演练 | 两轮隔离演练完成，未提交/未部署 | 历史 D1/D2 黄金副本；最新 D3/D4 候选库主线 55/55、repair 3/3；4 个项目、2 辆车辆、未解析/失效索引/缺失根 ID 均为 0 | 业务批准正式 D3/D4 manifest 与身份，完成锁/WAL/空间和守恒评估；配置阶段 target 后才允许生产执行 |
| BE-ID-01 | 主数据/进销存双写 | 本地完成 | 商品、仓库、客户、项目、供应商、物流商 ID与快照贯穿 DTO/实体/服务 | 共享环境同名不同 ID、改名不偏移和历史数据验证 |
| BE-ID-02 | 来源链与财务 typed identity | 本地完成 | 所有来源 ID、typed party、结算主体继承和不可变校验；物流稳定来源 91/91 | 共享环境来源冲突、跨主体、容量并发与真实状态流验证 |
| BE-ID-03 | 报表与候选 Switch | 本地完成 | 库存/出入库/应收应付按 ID；供应商候选 126/126、客户/项目/物流商 128/128、销售 117/117 | 生产 SQL计划、历史守恒、权限和真实分页 total 验证 |
| FE-ID-01 | API 边界 | 本地完成 | `EntityId` decoder、领域 DTO、保存白名单、草稿、直接来源和审计 actor | 修复 E2E selector/fixture 契约并全量重跑；共享环境嵌套 ID逐字符往返 |
| FE-ID-02 | 交互链 | 本地完成 | option、表单、父单导入、候选分页、批量生成、草稿和缓存键；Statement 109/109 | 修复导航、overlay、路由 marker 与 API Key 测试数据契约；验证真实权限与软删除回显 |
| QA-ID-01 | 定向回归 | 本地静态与迁移定向完成；完整真实 E2E 尚未通过 | 关键领域定向测试和 18 类 PostgreSQL 联合 62/62；release D3/D4 契约 8/8；开发测试副本 V56 与认证链通过，完整 E2E 可复核结果 25 passed、57 failed、2 skipped | 归档并重跑修复后的完整 E2E，核对采购订单导入、附件 smoke、live API Key 候选后再进入共享环境和生产副本发布门禁 |
| QA-ID-02 | 本地全量收口 | 完成 | 后端空 V55 库 6184/6184 + verify；前端 6181 passed + 4 skipped；TS/lint/build/audit；Flyway 空库；diff check | 发布级门禁独立完成后才可整体验收 |

任何后续发现的 schema 缺口必须登记目标表并从 V57+ 前滚，不得改写 V20–V56，也不得在业务代码中用名称 fallback 掩盖。

### 14.3 从本地冻结到发布完成的执行顺序

1. 当前差异和第 11.9 节本地全量证据已冻结并提交；完成受控评审后才允许远端合并。
2. 核验生产雪花机器号、时钟回拨、部署 target 和失败关闭配置。
3. 对历史数据执行只读扫描，形成错绑、空值、孤儿、冲突、逐单和全局守恒清单，并经业务确认清洗方案。
4. 以最新快照候选库已完成的 `V12–V29 → D3 → V30–V47 → D4 → V48–V55` 为基础，补齐锁等待、WAL、索引空间、逐单/全局守恒和回滚/前滚决策；V1→V55 仅用于空库门禁。
5. 补齐采购订单导入、附件 smoke、live API Key 详情的测试候选数据，重跑 3 个 skip；保持三条 Statement 流程使用编辑器内 canonical 往来方与父单导入。
6. 在共享环境分阶段执行 `migrate/validate`、应用启动、保存/回读、权限、软删除和真实业务状态流冒烟。
7. 观察生产 fallback 连续一个完整业务周期为 0，并验证 Dashboard、告警、备份恢复和回滚演练。
8. 全部发布门禁通过后再执行正式发布/部署并更新本文状态；本地证据不得替代任何目标环境证据。
...
5. 确认生产数据规模和可用维护窗口，以决定索引使用普通创建还是非事务并发创建。
6. 确认物流来源占用粒度是整张销售出库单还是单条出库明细，以及物流单软删除后是否允许重新导入；未确认前不增加错误的全局唯一约束。
7. 确认采购/销售合同是否具有结算主体和仓库语义；当前不存在的语义不为了字段统一而强行增加。
8. 车辆 ID及车辆/物流商组合归属已纳入 V48–V49并完成本地验证；仅 `material_category_id` 是否前滚增加仍需单独评审。
...
| 索引创建方式 | DBA/运维 + 后端 | 未取得规模与窗口数据时不在共享环境执行大表普通建索引 | E1/B1 正式迁移 |
...
- [x] 两份受限生产一致性快照的隔离副本已分别按 V29/D1/V47/D2/V55 和 V29/D3/V47/D4/V55 顺序迁移；最新候选库主线 55/55、repair 3/3，身份/约束/索引扫描通过。该项不代表 D3/D4 身份已获业务批准或已完成生产发布。
...
- [ ] 稳定身份 E2E（可复核历史 8/76/0、25/57/2；修复后的运行时观察未形成独立完整归档）、生产副本锁/WAL/空间与业务守恒、共享环境 migrate/validate、应用冒烟、生产备份恢复和发布演练全部通过。
