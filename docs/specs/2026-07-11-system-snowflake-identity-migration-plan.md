...
| 项目 | 内容 |
| --- | --- |
| 状态 | In Progress；本地代码、V20–V55 迁移链和主要本地质量门禁已完成，生产副本、共享环境、fallback 周期、E2E 与正式发布门禁尚未完成 |
| 计划版本 | 2.6（V48–V55 Contract 补缺与本地全量门禁收口版） |
| 日期 | 2026-07-12 |
| 涉及仓库 | 后端 `leo`、前端 `aries` |
| 审计基线 | `leo@99703021`、`aries@1d247de4` |
| 当前数据库版本 | Git 已受控至 Flyway V19；当前工作区已有未提交、未部署的 V20–V55；V20–V46 的 27 项与 V47–V55 的 9 项分别由独立 SHA-256 manifest 冻结；下一数据库变更只能新增 V56+ |
| 核心决策 | 复用现有 `BIGINT` 雪花主键作为唯一内部身份，不新增 UUID 或平行身份体系 |
| 当前操作边界 | `leo/main` ahead 4、414 个变更路径；`aries/main` ahead 13、160 个变更路径；均按用户指令保留当前工作树；两仓均未显示落后或分叉；未拉取、回滚、提交、推送、部署或直接修改共享库 |

### 1.1 当前实施进度（2026-07-12 收口快照）

当前两个仓库都处于未提交的并行实施工作区。下表中的“完成”只代表冻结工作树上的本地实现和本地自动化证据，
不代表已合并、已部署或已通过生产发布门禁。

| 工作包 | 当前状态 | 已有证据 | 进入下一阶段前仍需完成 |
| --- | --- | --- | --- |
| G0 雪花根基 | 本地实现完成 | 预分配失败关闭、本机生成、Long 字符串 JSON 与更新 ID 不变契约纳入后端全量 6177 项 | 生产机器号清单、时钟回拨告警与部署配置核验 |
| G1 客户/项目错写止血 | 本地实现完成 | 客户/项目分别解析，ID 碰撞、跨客户、同名不同 ID 与快照冲突回归通过 | 历史错绑只读扫描、业务清洗确认、共享环境验证 |
| E1/B1/C1 数据迁移 | V20–V55 本地脚本与契约完成，文件冻结 | Final Contract 5/5、Forward Contract 9/9、Constraint Contract 8/8、checksum 2/2；`leo_test` 到 V55；临时 PostgreSQL 16.14 空库 V1→V55 migrate/validate 通过 | 真实 `pg_index` 与锁影响复核、匿名化生产副本演练、共享环境分阶段迁移和数据守恒报告 |
| A1 后端双写/读模型 | 本地实现完成 | 物流稳定来源 91/91；供应商候选 126/126；客户/项目/物流商候选 128/128；销售候选 117/117；后端全量 6177/6177 | 历史数据、权限和真实状态流的共享环境验证；生产 fallback 观察 |
| A2 前端 API/交互链 | 本地实现完成 | 三类 Statement 6 文件 109/109；全量 Vitest 6180 passed、4 skipped；typecheck、Biome/ESLint、生产构建和生产依赖审计通过 | 稳定身份 E2E、共享环境保存/回读、真实权限与软删除行为验证 |
| QA 本地门禁 | 完成 | 699 个测试源编译；18 类联合 62/62；`mvn test` 与 `mvn verify` 均为 6177/6177；Checkstyle 0 violation、JaCoCo 达标；两仓 diff check 与部署脚本语法通过 | 现有 SpotBugs 51 个高等级告警另行治理，不冒充已通过；补齐 E2E 和发布级门禁 |
| 发布门禁 | 未完成 | 本地证据已冻结并记录 | 生产机器号/时钟、历史清洗、真实 `pg_index`、生产副本、共享环境、fallback 完整周期、Dashboard/告警/回滚、E2E、正式发布与部署 |
...
- 冻结工作树后端：`test-compile` 成功编译 699 个测试源；18 类联合 62/62；`mvn test` 6177/6177，0 failure/error/skip；`mvn verify` 再次 6177/6177 并通过 Checkstyle、打包和 JaCoCo 门禁。
- 冻结工作树前端：Vitest 461 个文件通过、1 个文件跳过；6180 项通过、4 项跳过；`pnpm typecheck`、Biome/ESLint、`pnpm build`、`pnpm audit --prod` 均通过。
...
`pg_index` 的列序、谓词、重复索引与锁影响复核；匿名化生产副本的迁移耗时、锁等待、WAL 与索引空间演练；共享环境
`migrate/validate`、应用冒烟与真实状态流；生产 fallback 连续完整业务周期为 0；Dashboard、告警、备份恢复和回滚演练；稳定身份 E2E；
...
| 前端稳定身份链 | 本地实现与全量门禁完成；Statement 109/109，Vitest 6180 passed + 4 skipped，typecheck/lint/build/audit 通过 | 补稳定身份 E2E 和共享环境保存/回读 |
| V20–V55 冻结证据 | 两份独立 SHA-256 manifest、迁移契约、`leo_test` V55和临时空库 V1→V55 均通过 | 将迁移与 manifest 纳入受控 Git历史；在真实 `pg_index`、生产副本和共享环境复核 |
| 本地全量门禁 | 后端 6177/6177、`mvn verify`、前端全量、本地构建/审计、两仓 diff check 均通过 | 仅剩 E2E 与生产/共享环境发布门禁；不复用任何历史结果 |
...
“内部主键固定”只代表实体自身的 `id` 不随编辑变化。当前本地代码与 V20–V55 已让既有雪花 ID贯穿数据库外键、
...
草稿、路由和候选链统一字符串 `EntityId`，unsafe number 在边界失败关闭，并由 6180 项前端全量测试及 TypeScript 门禁覆盖。
...
- 当前工作区 V20–V55 已完成跨领域契约串联，并按冻结迁移文件管理；V20–V46 与 V47–V55 的独立 SHA-256 快照已建立，后续修正只能从 V56 前滚。
...
V20–V55 文件当前已在工作区形成完整本地版本线，但尚未提交或部署；为避免并行 Agent 和后续环境产生不同 checksum，
...
清单不能证明文件在快照建立前从未改写，也不能替代将文件纳入受控 Git历史。以下内容记录实际文件与后续 V56+ 前滚边界。

| 范围 | 当前状态 | 执行许可 |
| --- | --- | --- |
| V20 | 文件存在，且已有测试执行证据 | 不得改写；问题用 V56+ 前滚 |
...
| V56+ | 尚未创建 | 只用于未来前滚修复或满足发布 C门禁后的兼容清理；按单一职责继续递增 |
...
| 后续清理（最早 V56） | 只移除已确认无命中的名称/编码/单号身份回退和无用索引；永久保留快照 | V47–V55 补缺完成、发布 C 完成且具备回滚证据；版本号以届时最大版本 + 1 为准 |
...
### 11.9 版本 2.6 本地冻结工作树证据

以下结果均来自 2026-07-12 冻结后的当前工作树，不复用此前 6020 项历史结果：

- 后端编译：主源码 840 个、测试源码 699 个编译成功。
- 后端定向：物流稳定来源 91/91、供应商候选 126/126、客户/项目/物流商候选 128/128、销售候选 117/117；最终 PostgreSQL 夹具联合 18 类 62/62。
- 后端全量：`mvn -B -ntp test` 为 6177/6177，0 failure、0 error、0 skip；`mvn -B -ntp verify` 再次 6177/6177，Checkstyle 0 violation、JaCoCo coverage check 达标并 BUILD SUCCESS。
- 迁移契约：Final Contract 5/5、Forward Contract 9/9、Constraint Contract 8/8、两份 checksum 契约 2/2。
- 数据库：本地 `leo_test` 当前为 V55并成功 validate 55 项；临时 PostgreSQL 16.14 空库 V1→V55 migrate/validate 55 项成功。
- 前端定向：三类 Statement 6 个文件 109/109；批量对账、父单导入、候选 current record和稳定 ID筛选均纳入全量回归。
- 前端全量：461 个 Vitest 文件通过、1 个文件跳过；6180 项通过、4 项跳过；`pnpm typecheck`、Biome/ESLint、`pnpm build` 与 `pnpm audit --prod` 通过。
- Ant Design：全仓 lint 扫描 1008 个文件，0 deprecated、0 a11y、0 usage；保留 1 条非阻断性能提示。
- 静态检查：`leo`、`aries` 的 `git diff --check` 和两个部署触发脚本 `bash -n` 通过。

未通过或未执行的发布门禁：当前 `test:e2e:mock` 没有 API mock，仍代理本地 11211 后端，稳定身份 E2E因后端未启动而
`ECONNREFUSED`；匿名化生产副本、历史脏数据/守恒、真实 `pg_index` 与锁/WAL/空间、共享环境、生产 fallback 周期、
Dashboard/告警、备份恢复、回滚演练和正式发布/部署均未执行。SpotBugs 仍有 51 个既有高等级告警，不能写成已通过。

## 12. 验收标准

全部满足后，才能对外宣称“全系统稳定身份固定完成”：

当前仅第 9 项的本地质量门禁已满足；第 1–8、10–12 项仍需生产历史数据、共享环境、fallback 或发布证据，
...
当前工作区已经完成“本地代码、迁移草案与本地质量门禁闭环”，未执行共享环境迁移。后续状态按本地完成与发布待验证分离：

| 任务 ID | 范围 | 当前状态 | 具体产物 | 完成门禁 |
| --- | --- | --- | --- | --- |
| DB-ID-01 | V20–V55 冻结迁移审查 | 本地完成 | 两份独立 checksum manifest；Final 5/5、Forward 9/9、Constraint 8/8；`leo_test` V55和空库 V1→V55 | 真实 `pg_index`、匿名化生产副本与共享环境验证；manifest 最终纳入受控 Git历史 |
...
| FE-ID-01 | API 边界 | 本地完成 | `EntityId` decoder、领域 DTO、保存白名单、草稿、直接来源和审计 actor | E2E 与共享环境嵌套 ID逐字符往返 |
| FE-ID-02 | 交互链 | 本地完成 | option、表单、父单导入、候选分页、批量生成、草稿和缓存键；Statement 109/109 | 稳定身份 E2E、真实权限与软删除回显 |
| QA-ID-01 | 定向回归 | 本地完成 | 关键领域定向测试和 18 类 PostgreSQL 联合 62/62 | E2E、共享环境与匿名生产副本门禁 |
| QA-ID-02 | 本地全量收口 | 完成 | 后端 6177/6177 + verify；前端 6180 passed + 4 skipped；TS/lint/build/audit；Flyway 空库；diff check | 发布级门禁独立完成后才可整体验收 |

任何后续发现的 schema 缺口必须登记目标表并从 V56+ 前滚，不得改写 V20–V55，也不得在业务代码中用名称 fallback 掩盖。

### 14.3 从本地冻结到发布完成的执行顺序

1. 冻结当前差异并保存第 11.9 节本地全量证据；在受控评审后才允许提交或合并。
2. 核验生产雪花机器号、时钟回拨、部署 target 和失败关闭配置。
3. 对历史数据执行只读扫描，形成错绑、空值、孤儿、冲突、逐单和全局守恒清单，并经业务确认清洗方案。
4. 复核真实 `pg_index`，在匿名化生产副本执行 V1→V55演练，记录耗时、锁等待、WAL、索引空间和回滚/前滚决策。
5. 在共享环境分阶段执行 `migrate/validate`、应用启动、保存/回读、权限、软删除和真实业务状态流冒烟。
6. 运行稳定身份 E2E；观察生产 fallback 连续一个完整业务周期为 0，并验证 Dashboard、告警、备份恢复和回滚演练。
7. 全部发布门禁通过后再执行正式发布/部署并更新本文状态；本地证据不得替代任何目标环境证据。
...
| 商品类别 ID P2 | 产品 + 架构 | 不纳入当前 V20–V55，单独评审后从 V56+ 前滚增加 | 不阻塞核心稳定身份；车辆 ID已由 V48–V49处理 |
...
- [x] 本地后端 `test/verify` 6177/6177、前端 Vitest 6180 passed + 4 skipped、TypeScript、Biome/ESLint、关键 PostgreSQL、本地生产构建、生产依赖审计、`leo_test`/空库迁移和两仓 diff check 已通过。
- [ ] 稳定身份 E2E、匿名化生产副本、共享环境 migrate/validate、应用冒烟、生产备份恢复和发布演练全部通过。

只有以上项目全部完成，且 V38–V55（若版本继续前滚则到届时最新 Contract 版本）约束已验证，才允许将本文状态从 `In Progress` 改为 `Implemented`，并对外宣称
“全系统雪花 ID 稳定身份固定完成”。
