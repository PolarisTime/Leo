...
| 当前数据库版本 | 本地 Git 已纳入 Flyway V1–V55但尚未推送或部署；V20–V46 的 27 项与 V47–V55 的 9 项分别由独立 SHA-256 manifest 冻结；下一数据库变更只能新增 V56+ |
| 核心决策 | 复用现有 `BIGINT` 雪花主键作为唯一内部身份，不新增 UUID 或平行身份体系 |
| 当前操作边界 | `leo/main`、`aries/main` 均领先远端且工作树干净；已创建本地实现提交 `leo@39f60d2`、`aries@6df06d5`；未拉取、回滚、推送、部署或直接修改共享库 |

### 1.1 当前实施进度（2026-07-12 收口快照）

当前两个仓库的实施差异已提交到本地 Git但尚未推送。下表中的“完成”只代表冻结工作树上的本地实现和本地自动化证据，
...
| V20–V55 冻结证据 | 两份独立 SHA-256 manifest、迁移契约、`leo_test` V55和临时空库 V1→V55 均通过，已纳入本地提交 | 完成代码评审与远端合并；在真实 `pg_index`、生产副本和共享环境复核 |
...
清单不能证明文件在快照建立前从未改写；当前本地提交仍需经过代码评审和远端合并。以下内容记录实际文件与后续 V56+ 前滚边界。
...
| --- | --- | --- | --- | --- |
| DB-ID-01 | V20–V55 冻结迁移审查 | 本地完成并已提交 | 两份独立 checksum manifest；Final 5/5、Forward 9/9、Constraint 8/8；`leo_test` V55和空库 V1→V55 | 代码评审与远端合并；真实 `pg_index`、匿名化生产副本与共享环境验证 |
