...
本节记录原审查之后开展的采购退款与全系统财务链路专项整改。详细事件模型、迁移预检和上线清单见
[`2026-07-11-finance-audit.md`](2026-07-11-finance-audit.md)。本节中的“已实现”指当前审计工作区已有对应代码和测试，
不表示开发库或生产库已执行迁移。本轮后端全量单测结果见下方，发布前迁移与其他质量门禁仍须独立完成。

| 范围 | 当前实现 | 关键约束 | 验证状态 |
| --- | --- | --- | --- |
| 采购退款 | 以采购订单原始数量、理论重量和金额为上限，扣减处于有效状态的实际采购入库数量、过磅重量和结算金额，生成未入库差额退款 | 一张采购订单最多一张活动退款单；审核退款前要求采购预付款足额支付；退款金额不直接重复记入应收应付 | 定向测试已覆盖计算、生命周期和真实 PostgreSQL 账簿场景；已纳入本轮全量单测 |
| 采购预付款 | 付款单新增独立用途 `PURCHASE_PREPAYMENT`，保存来源采购订单、供应商稳定编码及结算主体快照；来源使用专用分页候选接口 | 仅已审核或完成采购的订单可选；累计已付款金额不得超过订单原始金额；后续只能核销同供应商编码、同结算主体的已确认供应商对账单 | 强制编译已通过；预付款候选及 V17/V18 相关 19 个定向测试类曾通过；已纳入本轮全量单测 |
| 供应商退款到账 | 新增供应商退款到账单，允许同一采购退款分次到账 | 只接受已审核采购退款；累计到账不得超过退款金额；供应商编码与结算主体从退款单继承 | 服务、并发容量与应收应付 PostgreSQL 测试已覆盖；已纳入本轮全量单测 |
| 应收应付账簿 | 实际采购入库确认应付，采购预付款作为结算事件，供应商退款到账作为 `SETTLEMENT_REVERSAL` 结算冲回事件 | 采购退款单本身不是重复确认事件；供应商退款直接归入供应商稳定编码名下；按结算主体独立汇总、明细和账龄 | 仓储与 PostgreSQL 定向测试已覆盖；已纳入本轮全量单测 |
| 下游保护 | 采购入库、销售出库、物流单以及客户/供应商/物流对账单增加反审核、删除和来源变更保护 | 已存在退款、到账、收付款核销或下游对账关系时失败闭合，禁止改写上游事实 | 单元与契约测试已补充；已纳入本轮全量单测 |
| 物流商稳定身份 | 物流单新增 `carrierCode`，物流对账从来源物流单继承编码；选项、请求响应与分页筛选传递编码 | 同名不同编码不得合并；核心写链路不再按名称任取第一条；业务引用后禁止修改物流商编码 | 前端 `carrierCode` 闭环 131 项定向测试曾通过；后端定向测试及本轮全量单测通过 |

### 财务专项最终单测结果

执行 `mvn -q test`，退出码为 0。Surefire XML 汇总为 671 份报告、5995 项测试，Failures 0、Errors 0、
Skipped 0，实际通过 5995 项。本结果准确对应 Maven `test` 生命周期，不应表述为 `verify`，也不替代发布前的
Checkstyle、JaCoCo、Flyway validate、生产数据迁移演练和应用冒烟。

### 财务记账口径

1. 采购应付只以有效采购入库明细的 `amount + weightAdjustmentAmount` 确认，不再以采购订单原始金额或采购退款单重复确认。
2. 采购预付款按付款事实记为应付结算；已分配部分进入对应供应商对账状态，未分配余额保留为未对账结算事件。
3. 供应商实际退款到账记为供应商名下的结算冲回。草稿到账单不入账，已收款到账单才影响余额。
4. 供应商稳定编码、物流商稳定编码和结算主体共同构成隔离维度；同名记录或同一往来方的不同结算主体不得相互抵扣。
5. 发票、对账单和退款业务单据负责证据链与分组，不额外制造与实际出入库、收付款重复的应收应付确认事件。

### V12 至 V18 迁移状态与上线边界

| 环境/版本 | 审计时状态 | 约束 |
| --- | --- | --- |
| 测试库 | 测试启动已自动迁移并验证至 `V18` | 仅代表测试数据库，不代表共享环境已升级 |
| 共享开发库 | `V12` 已执行；`V13` 至 `V18` 未手工应用 | `V12` 已成为不可变历史，严禁改写；后续问题只能新增更高版本迁移 |
| 生产库 | `V13` 至 `V18` 尚未应用 | 上线前必须核对实际 `flyway_schema_history`，执行 `migrate` 与 `validate`，不得绕过 Flyway 手工写入 |

审计时 V12 至 V18 尚未纳入 Git，Git 历史不能证明共享开发库已执行 V12 的 checksum 与当前文件一致；
部署前必须以共享环境的 `flyway_schema_history` 和 `flyway:validate` 复核。任何差异都不得通过改写 V12 处理，只能新增更高版本迁移。

`V18` 对物流单和物流对账单采用失败闭合回填：同名多编码、无法唯一匹配、来源编码冲突或来源聚合出多个编码时，
迁移会抛出异常并整体回滚。回填、`NOT NULL`、检查约束和索引创建会扫描并锁定相关表，应安排维护窗口并先在生产数据副本预检。
迁移完成后，旧版本服务无法继续写入缺少 `carrier_code` 的物流单，必须协调停写、迁移和新版本切换，禁止新旧版本混合写入。

物流商编码在主数据表上采用“仅活动记录唯一”的部分唯一索引，不能作为普通外键的被引用唯一键，因此 V18 未添加普通外键；
完整性由服务端编码解析、非空/非空白约束以及编码被业务引用后的生命周期保护共同保证。
...
### 修复阶段最终验证结果（截至 V11 的历史批次）

以下结果只覆盖 2026-07-10 已落地的 `V1` 至 `V11` 批次，不覆盖 2026-07-11 财务专项的 `V12` 至 `V18`、
采购退款、采购预付款、供应商退款到账和物流商稳定身份改造。本轮准确结果为上方 `mvn -q test` 的 5995 项通过，
不得沿用本节数字，也不得把本轮命令写成 `verify`。
