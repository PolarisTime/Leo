...
class PrintScriptServiceTest {
    ...
    @Test
    void shouldExposeChargeItemsSectionsAndPurchaseChargeTotalsWithoutChangingProductTotals() {
        JdbcTemplate jdbc = mock(JdbcTemplate.class);
        PrintScriptService service = printScriptService(repository("purchase-order"), jdbc);
        ...
        when(jdbc.queryForList(org.mockito.ArgumentMatchers.contains("po_purchase_order_item"), eq(1L))).thenReturn(List.of(Map.of(
                "line_no", 1,
                "brand", "沙钢",
                "quantity", 2,
                "weight_ton", new BigDecimal("2.000"),
        ...
        when(jdbc.queryForList(org.mockito.ArgumentMatchers.contains("bd_document_charge_item"), eq("purchase-order"), eq(1L))).thenReturn(List.of(
                Map.of(
                        "line_no", 1,
                        "charge_name", "卸货费",
                ...
                Map.of(
                        "line_no", 2,
                        "charge_name", "代收服务费",
                        "charge_direction", "RECEIVABLE",
                        "amount", new BigDecimal("1.11"),
                        "billable", true
                ),
                Map.of(
                        "line_no", 3,
                        "charge_name", "内部费用",
                        "charge_direction", "INTERNAL",
        ...
        Map<?, ?> data = (Map<?, ?>) result.get("data");
        assertThat(data.get("totalAmount")).isEqualTo("100.00");
        assertThat(data.get("totalWeight")).isEqualTo("2.000");
        assertThat(data.get("moduleKey")).isEqualTo("purchase-order");
        assertThat(data.get("totalChargeAmount")).isEqualTo("12.34");
        assertThat(data.get("payableAmount")).isEqualTo("112.34");
        assertThat(data.get("receivableAmount")).isEqualTo("100.00");

        List<?> chargeItems = (List<?>) result.get("chargeItems");
        assertThat(chargeItems).hasSize(4);
        Map<?, ?> firstChargeItem = (Map<?, ?>) chargeItems.getFirst();
        assertThat(firstChargeItem.get("chargeName")).isEqualTo("卸货费");
        assertThat(firstChargeItem.get("amount")).isEqualTo("12.34");

        Map<?, ?> sections = (Map<?, ?>) result.get("sections");
        assertThat(sections.get("items")).isEqualTo(result.get("items"));
        assertThat(sections.get("chargeItems")).isEqualTo(result.get("chargeItems"));
    }
