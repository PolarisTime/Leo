...
-- Seed watermark system settings
...
SELECT REPLACE(gen_random_uuid()::text, '-', ''), 'UI_WATERMARK_ENABLED', '全局水印开关', '水印', 'SYS', 'NONE', 6, 'NEVER', 'ON', '禁用', '开启后页面显示全局水印，内容为空时默认显示登录用户名', NOW(), NOW()
