...
ALTER TABLE sys_user ADD CONSTRAINT chk_sys_user_status CHECK (status IN ('正常', '禁用'));
...
ALTER TABLE auth_api_key ADD CONSTRAINT chk_api_key_status CHECK (status IN ('正常', '禁用'));
