...
@Repository
public class FreightBillSalesOrderCandidateQueryRepository {
    ...
    private static final String CANDIDATE_CTE = """
            WITH candidate_orders AS (
                ...
                WHERE sales_order.deleted_flag = FALSE
                  AND sales_order.status IN (:allowedStatuses)
                      ...
                      WHERE source_item.order_id = sales_order.id
                        AND COALESCE(source_item.quantity, 0) > COALESCE((
                            ...
                            WHERE item.source_sales_order_item_id = source_item.id
                              AND (:currentRecordId IS NULL OR bill.id <> :currentRecordId)
                        ), 0)
                  )
            )
            """;
    ...
    public FreightBillSalesOrderCandidateQueryRepository(NamedParameterJdbcTemplate jdbcTemplate) {
    ...
    private MapSqlParameterSource parameters(PageQuery query, PageFilter filter) {
        return new MapSqlParameterSource()
                .addValue("allowedStatuses", ALLOWED_STATUSES)
