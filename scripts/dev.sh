...
do_stop() {
  stop_process "前端(dev)" "$FRONTEND_PORT" "$FRONTEND_PID_FILE"
...
start_frontend() {
  ...
  echo "[dev] 启动前端 ..."
  setsid bash "$ARIES_DIR/scripts/frontend/start-dev.sh" > "$FRONTEND_LOG" 2>&1 < /dev/null &
  echo "$!" > "$FRONTEND_PID_FILE"
  if ! wait_for_http "前端(dev)" "http://127.0.0.1:$FRONTEND_PORT" "$FRONTEND_LOG" 60; then
    stop_process "前端(dev)" "$FRONTEND_PORT" "$FRONTEND_PID_FILE"
    return 1
  fi
}
